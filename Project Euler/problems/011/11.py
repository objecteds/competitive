...
for i in range(20):
    for j in range(20):
        # check horizontal 4
        if j <= 16:
            h = int(grid[i][j]) * int(grid[i][j+1]) * int(grid[i][j+2]) * int(grid[i][j+3])
            if h > greatest:
                greatest = h

        # check vertical 4
        if i <= 16:
            v = int(grid[i][j]) * int(grid[i+1][j]) * int(grid[i+2][j]) * int(grid[i+3][j])
            if v > greatest:
                greatest = v

        if j <= 16 and i <= 16:
            # check diagonal down
            dd = int(grid[i][j]) * int(grid[i+1][j+1]) * int(grid[i+2][j+2]) * int(grid[i+3][j+3])
            if dd > greatest:
                greatest = dd
            
            # check diagonal up
            du = int(grid[i+3][j]) * int(grid[i+2][j+1]) * int(grid[i+1][j+2]) * int(grid[i][j+3])
            if du > greatest:
                greatest = du
